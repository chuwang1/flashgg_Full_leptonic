...
namespace flashgg {
  ...
  class HHWWggFLTagProducer : public EDProducer
  {
  public:
    //---typedef
    ...
    //---ctors
    // HHWWggFLTagProducer();
    HHWWggFLTagProducer( const ParameterSet & );
  ...
  private:
    ...
    bool doHHWWggFLTagCutFlowAnalysis_;
    bool SaveOthers_;
    bool Savejet;
    //FullLep
    double lep1ptThre_;
    ...
    //---standard
    HHWWggFLTagProducer::HHWWggFLTagProducer( const ParameterSet & pSet):
    ...
    // idSelector_( pSet.getParameter<ParameterSet> ( "idSelection" ), cc_ )
    {
      ...
      doHHWWggFLTagCutFlowAnalysis_ = pSet.getParameter<bool>( "doHHWWggFLTagCutFlowAnalysis");
      // produces<vector<HHWWggFLTag>>();
      for (auto & systname : systematicsLabels) {
          produces<vector<HHWWggFLTag>>(systname);
      }
      produces<vector<TagTruthBase>>();
      // cout << "**************************** in HHWWggFLTagProducer.cc **********************************************" << endl;
    }

    // bool HHWWggFLTagProducer::PassMVASelections()
    // {
// 
    // }
    ...
    bool HHWWggFLTagProducer::checkPassMVAs( const flashgg::Photon*& leading_photon, const flashgg::Photon*& subleading_photon, edm::Ptr<reco::Vertex>& diphoton_vertex,double EB_MVA,double EE_MVA){
    ...
    void HHWWggFLTagProducer::produce( Event &event, const EventSetup & )
    {
      cout<<"start Tag HHWWggFLTagProducer"<<endl;
      // cout << "[HHWWggFLTagProducer.cc] - Beginning of HHWWggFLTagProducer::produce" << endl;
      Savejet=1;
...
//-----------------------------------------------------------------------------------------------------------
      ...
      if(doHHWWggFLTagCutFlowAnalysis_) {Cut_Variables[0] = 1.0;}
      //for(int i =0;i<5;i++){
      ...
      //chuw/for (unsigned int diphoton_idx = 0; diphoton_idx < diPhotonTokens_.size(); diphoton_idx++) { //looping over all diphoton systematics
        ...
        std::unique_ptr<vector<HHWWggFLTag> > tags( new vector<HHWWggFLTag> );
        ...
        for( unsigned int diphoIndex = 0; diphoIndex < 1; diphoIndex++ ) { // only look at highest pt dipho
          ...
          diphoton_vertex = dipho->vtx(); 
          cout<<"dipho"<<endl;
          if(doHHWWggFLTagCutFlowAnalysis_ && 1){
         ...
         /* if(doHHWWggFLTagCutFlowAnalysis_){
          for(double i=-1.;i<1.;i=i+0.01){
            double EB_MVA=i;
            for(double j=-1.;j<1.;j=j+0.01){
            double EE_MVA=j;
...
//          unsigned int jet_token_index = jet_col_idx*inputJetsCollSize_+vtx;
        ...
        cout<<"Savejet"<<Savejet<<"Jets:"<<Jets_->size()<<endl;
         ...
         // chuw//n_good_jets = tagJets.size();
          ...
          if(doHHWWggFLTagCutFlowAnalysis_ && 0){
          ...
          if ( (n_good_leptons >=2 ) && (theMET->getCorPt() >= MetPtThreshold_) && num_FL_dr>=1 ){
            ...
            if (n_good_electrons >= 2){
              ...
              cout<<"MassT:"<<MassT<<"MassT_l2:"<<MassT_l2<<endl;
                ...
                HHWWggFLTag tag_obj(dipho, tag_electron1, tag_electron2, theMET,Cut_Variables); // HHWWggFLTag need to be updated
...
if(tag_muon2->pt() >= i && tag_muon1->pt() >= j && ((dipho->mass()<sig_mass_low_ && dipho->mass()>100.) || (dipho->mass()<180. && dipho->mass()>sig_mass_high_)) )
  lepPt2DCut->Fill(i,j);
              ...
              MassT_l2 = sqrt(2*Mu2LV.Pt()*theMET->pt()*(1-cos(abs(Mu2LV.Phi()-theMET->phi()))));
                ...
                HHWWggFLTag tag_obj(dipho, tag_muon1, tag_muon2, theMET,Cut_Variables);  
                tag_obj.setSystLabel(systLabel_);
            ...
            reco::Candidate::LorentzVector MuLV = tag_muon1->p4();
              ...
              if((((tag_electron1->pt()>=lep1ptThre_) && (tag_muon1->pt()>=lep2ptThre_))||((tag_muon1->pt()>=lep1ptThre_) && (tag_electron1->pt()>=lep2ptThre_))) && (tag_muon1->charge()*tag_electron1->charge()==-1) && (DiLepLV.M()>DiLepMassThre_) && (DiLepLV.Pt()>DiLepPtThre_) && (MassT_l2>MassTThre_) && (MassT>MassT_l2Thre_) && (Save==1.) &&(Savejet==1)){
                HHWWggFLTag tag_obj(dipho, tag_electron1, tag_muon1, theMET,Cut_Variables);
                tag_obj.setSystLabel(systLabel_);
          ...
          {
            if(doHHWWggFLTagCutFlowAnalysis_ && SaveOthers_)
            { 
               //diPhotons_size_h->Fill(1);
              ...
              {int catnum =3;
               ...
               HHWWggFLTag tag_obj(dipho,tag_electron,Cut_Variables);
               //HHWWggFLTag tag_obj(dipho,Cut_Variables);
//               std::cout<<"dipho1"<<std::endl;

               //diPhotons_size_h->Fill(Cut_Variables[0]);
               //HHWWggFLTag tag_obj(Cut_Variables);
              ...
              if(n_good_muons ==1 ){
                int catnum =3;
               Ptr<flashgg::Muon> tag_muon = goodMuons[0];
               HHWWggFLTag tag_obj(dipho,tag_muon,Cut_Variables);
               tag_obj.setDiPhotonIndex( diphoIndex );
               tag_obj.setMVA( -0.9 );
               tag_obj.setCategoryNumber( catnum );
               tags->push_back( tag_obj );
  ...
  //        diPhotons_size_h->Fill(Cut_Variables[0]);
    ...
    } // HHWWggFLTagProducer::produce

  } // namespace flashgg

  typedef flashgg::HHWWggFLTagProducer FlashggHHWWggFLTagProducer;
  DEFINE_FWK_MODULE( FlashggHHWWggFLTagProducer );
