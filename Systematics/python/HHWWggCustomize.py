...
class HHWWggCustomize():
    ...
    def __init__(self, process, customize, metaConditions):
        ...
        self.tagList = [ ["HHWWggFLTag",3] ] # 2 cats: 0: SL electron channel. 1: SL muon channel. 2: Untagged --> Meets no criteria but want to save event to output 
    ...
    def customizeTagSequence(self):
        ...
        # if self.customize.doHHWWggFLTagCutFlow: 
        if self.customize.doHHWWggFLTagCutFlow or self.customize.saveHHWWggFinalStateVars:  ##-- set true for either case because finalstate vars only saved during cutflow 
            self.process.flashggHHWWggFLTag.doHHWWggFLTagCutFlowAnalysis = cms.bool(True)

        if self.customize.doHHWWggNonResAnalysis: 
            self.process.flashggHHWWggFLTag.doHHWWggNonResAnalysis = cms.bool(True)

        # if self.customize.saveHHWWggGenVars: 
            # self.process.flashggHHWWggFLTag.saveHHWWggGenVars = cms.bool(True)

        # if self.customize.saveHHWWggFinalStateVars:
            # self.process.flashggHHWWggFLTag.saveHHWWggFinalStateVars = cms.bool(True)
    ...
    # def HHWWggFLTagRunSequence(self,systlabels,jetsystlabels,phosystlabels):
        # if self.customize.saveHHWWggGenVars:
            # self.addGenAnalysis()           
        # print'[HHWWggFLTagRunSequence]: Doing Nothing for HHWWgg'
    #    if self.customize.HHWWggFLTagsOnly: 
